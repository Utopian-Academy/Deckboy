...
Verified against installed **SDL 2.32.10** (vcpkg, `C:\Users\james\vcpkg`).
...
| Thing | State |
|-------|-------|
| vcpkg root | `C:\Users\james\vcpkg` (classic mode, `VCPKG_MANIFEST_MODE=OFF`) |
...
- [ ] **Install libav dev libs** (the gating prerequisite). Classic-mode vcpkg:
      ...
      run when convenient. **Ask James before running** (changes his toolchain).
