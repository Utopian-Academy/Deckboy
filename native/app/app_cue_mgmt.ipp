...
// ============================================================================
  ...
  // WHERE IT WAS DROPPED DECIDES WHICH PLAYLIST IT JOINS.
  //
  // This used to take only the path, and importPaths puts cues in the FOCUSED
  // deck -- so a file went wherever the highlight was, however carefully it
  // was aimed at a column. With one playlist that is indistinguishable from
  // working, which is why it survived until there were several.
  //
  // Dropping somewhere that is not a column keeps the old behaviour, because
  // a file dropped on the timeline still has to go somewhere obvious.
  void handleDropFile(const char* rawPath, float dropX, float dropY) {
    if (!rawPath) {
      return;
    }
    const int x = static_cast<int>(dropX);
    const int y = static_cast<int>(dropY);
    for (int di = 0; di < static_cast<int>(deckListClipRects_.size()); ++di) {
      const SDL_Rect& column = deckListClipRects_[di];
      if (column.w <= 0 || column.h <= 0 || !pointInRect(x, y, column)) {
        continue;
      }
      if (di != project_.focusedDeckIndex) {
        // THE DROP MOVES THE HIGHLIGHT, not the other way round. Doing this
        // before the import is what makes importPaths land in the right
        // playlist, and it is also what the operator expects to see.
  ...
  void addNdiSourceCue(const std::string& sourceName) {
    ...
    // WHETHER NDI CAN RUN AT ALL, checked here and reported in the toast at
    // the end of this function. Raising it now instead would be pointless:
    // the "cue added" toast below replaces whatever is on screen, so the
    // warning would appear and vanish inside the same call. Only the LAST
    // toast a function raises is ever seen.
    std::string ndiWhyNot;
    const bool ndiUsable =
      deckboy::platform::video::NdiInput::available(&ndiWhyNot);
    auto [rasterW, rasterH] = outputRenderSizeForOutput(project_.focusedOutputIndex);
    Cue cue;
    ...
    deck.selectedIndex = static_cast<int>(deck.cues.size()) - 1;
    onSelectionChanged();
    // NDI input is received natively now -- through the SDK, the way a
    // DeckLink input is captured. It used to go through ffmpeg's
    // libndi_newtek device, which upstream removed in 2021, so the cue was
    // created, named, and stayed blank for ever with nothing said. What can
    // still stop it is the NDI RUNTIME being absent, and that says so.
    if (ndiUsable) {
  ...
  void openSourceTypeMenu() {
    ...
    contextItems_.push_back({
      "  Lower Third (a name and a subtitle)",
      {0, 0, 0, 0},
      [this]() { addLowerThirdCue(); }});
    contextItems_.push_back({
