...
| `native/core/audio_effects.hpp` | The per-cue AUDIO chain: nine ordinary effects, the five that read `AudioEffectContext` (the picture, the geometry, the length, the video clock, the hold), and eight BENDS (crush, word, skip, rail, resolution, scrub, short, ouroboros) that damage the signal the way a corrupted file does. Runs on the audio thread between the cue's gain and the peak limiter. Verify with `--audio-fx-check` |
| Cross-domain effects | The loop runs both ways. On the picture side (`cue_effects.hpp`), `Databend` plays the frame through an audio chain and paints the result, and `Audioprint` draws the deck's played audio through the picture (`CueEffectContext::audioSamples`, filled only when `cueEffectStackDrawsWithAudio`). On the audio side, the `Audio` LFO shape follows `programAudioLevel01()` and the `Ouroboros` bend reads the post-effect picture. Put one of each on a cue and each bends the other |
...
| `native/platform/capture_backend.cpp/hpp` | Source capture (camera/window/screen). Windows WINDOW capture is Windows.Graphics.Capture via ffmpeg's `gfxcapture` (DPI-correct, follows the window, ignores occlusion), selected by HWND; gdigrab is the desktop/region path and the automatic fallback. WGC is REPAINT-driven, so the engine's `serviceWindowCaptureWatchdog` nudges a silent window and swaps to the fallback if nothing ever arrives |
...
| `tools/audit_encoding.py` | Fail on text that went into the source encoded TWICE and draws as mojibake (the effect arrows, then the Orientation button's degree sign — neither caught by a compiler, a test or a screenshot). A second, ADVISORY check lists non-ASCII inside drawn string literals |
...
| `tools/check_effects_offline.py` | Run every effect through `--effect-dump` on one picture: seconds, exactly repeatable, and `--sheet` writes a contact sheet. Use this while WRITING an effect -- the app-driven sweeps cost minutes and land on whatever frame the seek reached. `--params` is the only gate that catches a control which does nothing -- it caught SHATTER arriving completely inert after twelve days in shipped builds. All three modes (plain, `--params`, `--animation`) run in CI on every push as of 2026-09-17, on the Linux feature build |
